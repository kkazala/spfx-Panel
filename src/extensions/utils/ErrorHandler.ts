  ...
  if (hOP(e, "isHttpRequestError")) {
    ...
    const message =
      typeof data["odata.error"] === "object"
        ...
        : e.message;
    // use the status to determine a custom logging level
    const level: LogLevel =
      (<HttpRequestError>e).status === 404 ? LogLevel.Warning : LogLevel.Error;
    ...
    // not an HttpRequestError so just log message
    Logger.error(e);
  }
}
